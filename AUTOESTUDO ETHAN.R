...
dadosreceipt <- WDI(country = 'all', indicator = 'EG.ELC.ACCS.ZS')
dadosreceipt2023 <- WDI(country = 'all', indicator ='EG.ELC.ACCS.ZS', start = 2023, end = 2023)
dadosreceiptbrus <- WDI(country = paises, indicator ='EG.ELC.ACCS.ZS')
dadosreceiptbr <- WDI(country = 'BR', indicator = 'EG.ELC.ACCS.ZS')

# Painel
grafpainel <- ggplot(dadosreceipt, mapping = aes(y = EG.ELC.ACCS.ZS, x = year, color = country)) +
  geom_point() +
  labs(title = "Receitas de Turismo Internacional (WDI)",
       x = "Ano",
       y = "PIN") +
